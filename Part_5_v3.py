...
#            dm.setActuators(np.random.rand(19) * 2 - 1)
            actarray=np.zeros(19)
            actarray[0]=0.8
            dm.setActuators(actarray)
            time.sleep(0.2)
    ...
    # Normalize coordinates for the unit circle
    ...
    # Interpolate x and y displacements onto the grid separately
    points = reference_dots_normalized
    grid_points = np.c_[xv.flatten(), yv.flatten()]
    displacement_x_grid = griddata(points, displacements[:, 0], grid_points, method='cubic').reshape(xv.shape)
    displacement_y_grid = griddata(points, displacements[:, 1], grid_points, method='cubic').reshape(xv.shape)

    # Combine x and y displacements
    combined_displacement_grid = np.sqrt(displacement_x_grid**2 + displacement_y_grid**2)
    coeffs = zern.fit_cart_grid(combined_displacement_grid.flatten())[0]
    reconstructed_wavefront = zern.eval_grid(coeffs, matrix=True)
    ...
    plt.subplot(1, 2, 2)
    plt.plot(range(1, zern.nk + 1), coeffs, marker='.')
