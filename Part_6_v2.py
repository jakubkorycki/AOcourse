...
def calculate_C_matrix(slopes_data, voltages):
    C_alt, _, _, _ = np.linalg.lstsq(slopes_data, voltages, rcond=None)
    C_matrix = C_alt.T
...
def update_voltages(dm, current_voltages, voltage_correction):
    new_voltages = current_voltages - 0.5 * voltage_correction
...
def converge_to_zernike(dm, camera, reference_grid, C, zernike_coeffs, n_modes, max_iterations=10, tolerance=1e-6, ref_volt=None):
...
#    current_voltages = np.zeros(19)
    ...
    for iteration in range(max_iterations):
        ...
        try:
            print(f"1 slope difference: {np.sum(desired_slope_pattern - current_slopes)}")
#            unew = ucur - 0.2 * 0.5 * C@s_cur
            voltage_correction = 0.2*np.clip(calculate_desired_voltages(C, desired_slope_pattern - current_slopes), -1.0, 1.0)
            previous_slopes = current_slopes
        except:
            print(f"2 slope difference: {desired_slope_pattern - previous_slopes}")
            voltage_correction = 0.2*np.clip(calculate_desired_voltages(C, desired_slope_pattern - previous_slopes), -1.0, 1.0)
...
#             Matching grid and normalizing
            ...
            reference_grid_normalized, distorted_grid_normalized = normalize_grids(reference_grid, distorted_grid)
            slopes = get_slopes(reference_grid_normalized, distorted_grid_normalized)
            print(calculate_desired_voltages(C_matrix, slopes))
            print(rand_act)
        ...
        if False:
            ...
            zernike_coeffs[2] = 1
            avg_norm = np.average(np.linalg.norm(slopes_data, axis=1))
            
            desired_slopes = calculate_desired_slope_pattern(zernike_coeffs, reference_grid_normalized, n_modes)
#            desired_slopes_normalized = desired_slopes / (np.linalg.norm(desired_slopes) / avg_norm)
        ...
        if True:
            zernike_coeffs = np.zeros(n_modes)
            zernike_coeffs[0] = 1 # Normalize coefficients? #TODO!
            
            voltages, current_grid_normalized = converge_to_zernike(dm, sh_camera,\
                                                                    reference_grid,\
                                                                    C_matrix, \
                                                                    zernike_coeffs,\
                                                                    n_modes, \
